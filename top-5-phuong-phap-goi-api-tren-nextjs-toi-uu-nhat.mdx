...
| ----------- | ------------ | ------- | ---------- |
| `getServerSideProps` | Dữ liệu thay đổi liên tục | Dữ liệu luôn mới nhất | Chậm nếu API lâu |
| `getStaticProps` | 	Dữ liệu cố định hoặc ít thay đổi | Load nhanh, SEO tốt | Không phù hợp real-time |
| `API Route` | Cần bảo mật hoặc xử lý nhiều logic backend | Ẩn key API | Setup thêm route |
| `SWR` hoặc `React Query` | Tải dữ liệu trên client-side, cần cache | Dễ dùng, cache tốt | Không tối ưu SEO |
| ISR | Cần cả tốc độ và cập nhật định kỳ	| Tối ưu tốc độ và SEO | Cần thêm cấu hình |
